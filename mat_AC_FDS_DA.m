function [A,C,dAds,T,dA,ds_int] = mat_AC_FDS_DA(mat,param,N,s)
% mat_AC_FDS_DA
%
% Generate A and C matrices for Discrete Adjoint Finite Difference in the strong form 
%
% INPUTS: 
...
%% Unwrap param
[gam,zet,n,Ru,Rd,~,~,kus,kds,cu,cd] = unwrap_param(param);

%% Generate A, C, and dAds matrices
A    = M*D*V*D - M * zet * n * diag(exp(-s*t))     * h * wr.' * M * D;
C    = M/gam;
dAds =         - M * zet * n * diag(-t.*exp(-s*t)) * h * wr.' * M * D;

%% Apply the boundary conditions to A, C, and dAds
[A,C,dAds] = fun_bcs_strong(D,A,C,dAds,N,Ru,cu,kus,Rd,cd,kds,s);
...
if nargout >= 5
    ...
    elseif Rd == +1
        % homogenous neumann on full matrices
        dA.n(1,:)     = zeros(1,N+1);  % top row of dA.n    -> 0
        % homogenous neumann on split matrices
        dA.tl(1,:)    = zeros(1,N+1);  % top row of dA.tl   -> 0
        dA.hl(1,:)    = zeros(1,N+1);  % top row of dA.hl   -> 0
        dA.wrl(1)     = 0;             % top row of dA.wrl  -> 0
        dA.vl(1,:)    = zeros(1,N+1);  % top row of dA.vl   -> 0
        dA.kd         = 0;
        dA.mrul(1,:)  = zeros(1,N+1);  % top row of dA.mrul -> 0
        dA.mrpl(1,:)  = zeros(1,N+1);  % top row of dA.mrpr -> 0
        dA.frul(1,:)  = zeros(1,N+1);  % top row of dA.frul -> 0
        dA.frpl(1,:)  = zeros(1,N+1);  % top row of dA.frpr -> 0
        dA.qpul(1,:)  = zeros(1,N+1);  % top row of dA.qpul -> 0
        dA.qppl(1,:)  = zeros(1,N+1);  % top row of dA.qppr -> 0
    else
        % Robin on full matrices
        dA.n(1,:)     = zeros(1,N+1);  % top row of dA.n    -> 0
        % Robin on split matrices
        dA.tl(1,:)    = zeros(1,N+1);  % top row of dA.tl   -> 0
        dA.hl(1,:)    = zeros(1,N+1);  % top row of dA.hl   -> 0
        dA.wrl(1)     = 0;             % top row of dA.wrl  -> 0
        dA.vl(1,:)    = zeros(1,N+1);  % top row of dA.vl   -> 0
    ...
    if Ru == -1
        ...
        % homogenous dirichlet on split matrices
        dA.tl(N+1,:)   = zeros(1,N+1);  % bot row of dA.t    -> 0
        dA.tr(:,N+1)   = zeros(N+1,1);  % rit col of dA.t    -> 0
        dA.hl(N+1,:)   = zeros(1,N+1);  % bot row of dA.v    -> 0
        dA.hr(N+1)     = 0;             % rit col of dA.v    -> 0
        dA.wrl(N+1)    = 0;             % bot row of dA.w    -> 0
        dA.wrr(:,N+1)  = zeros(N+1,1);  % rit col of dA.w    -> 0
        dA.vl(N+1,:)   = zeros(1,N+1);  % bot row of dA.r    -> 0
        dA.vr(:,N+1)   = zeros(N+1,1);  % rit col of dA.r    -> 0
        dA.ku          = 0;
        dA.mrul(N+1,:) = zeros(1,N+1);  % bot row of dA.mu   -> 0
        dA.mrur(:,N+1) = zeros(N+1,1);  % rit col of dA.mu   -> 0
        dA.mrpl(N+1,:) = zeros(1,N+1);  % bot row of dA.mp   -> 0
        dA.mrpr(:,N+1) = zeros(N+1,1);  % rit col of dA.mp   -> 0
        dA.frul(N+1,:) = zeros(1,N+1);  % bot row of dA.fu   -> 0
        dA.frur(:,N+1) = zeros(N+1,1);  % rit col of dA.fu   -> 0
        dA.frpl(N+1,:) = zeros(1,N+1);  % bot row of dA.fp   -> 0
        dA.frpr(:,N+1) = zeros(N+1,1);  % rit col of dA.fp   -> 0
        dA.qpul(N+1,:) = zeros(1,N+1);  % bot row of dA.qu   -> 0
        dA.qpur(:,N+1) = zeros(N+1,1);  % rit col of dA.qu   -> 0
        dA.qppl(N+1,:) = zeros(1,N+1);  % bot row of dA.qp   -> 0
        dA.qppr(:,N+1) = zeros(N+1,1);  % rit col of dA.qp   -> 0
    elseif Ru == +1
        % homogenous neumann on full matrices
        dA.n(N+1,:)    = zeros(1,N+1);  % bot row of dA.n    -> 0
        % homogenous neumann on split matrices
