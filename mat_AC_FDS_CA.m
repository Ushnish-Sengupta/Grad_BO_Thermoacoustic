function [A,C,dAds,T] = mat_AC_FDS_CA(mat,param,N,conjs)
% mat_AC_FDS_CA
%
% Generate A and C matrices for Continuous Adjoint Finite Difference in the strong form 
%
% INPUTS: 
...
A    = M*D*V*D + M * zet * n * dwrdx * h' * diag(          exp(-conjs*conj(t))) * M;
C    = M/gam;
dAds =           M * zet * n * dwrdx * h' * diag(-conj(t).*exp(-conjs*conj(t))) * M;

%% Apply the boundary conditions to A, C, and dAds
[A,C,dAds] = fun_bcs_strong(D,A,C,dAds,N,Ru,cu,conj(kus),Rd,cd,conj(kds),conjs);
